 ...
 * converter.js
 *
 * FIX: Both downloadConverted() and sendConverterToEncoder() used the same
 * chunked atob() loop as decoder.js. Applied the same multiple-of-4 chunk
 * size guard to both. ATOB_CHUNK is defined in decoder.js (loaded first)
 * so we reuse it here; it equals 65536 which is already divisible by 4.
 */
...
async function convertHEICviaServer(file) {
  ...
  let response;
  try {
    response = await fetch('/api/convert-heic', { method: 'POST', body: formData });
  } catch (networkErr) {
    throw new Error(
  ...
  if (!response.ok) {
    let serverMsg = '';
    try { const json = await response.json(); serverMsg = json.error || ''; } catch (_) { }
    throw new Error(
      serverMsg || 'Server returned an error (' + response.status + '). Please try again.'
    );
  }
...
function blobToCanvas(blob, label) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      ...
      reject(new Error('Could not render ' + (label || 'image') + ' — the file may be corrupt.'));
    };
...
function updateConverterPreview() {
  ...
  document.getElementById('convQualityRow').style.display = (fmt === 'png') ? 'none' : 'flex';
  ...
  // Track conversion
  if (typeof trackEvent === 'function') {
    trackEvent("image_converted", "Feature", "Converter", { format: fmt });
  }
...
// ── Shared helper: dataURL → Blob (chunked, multiple-of-4 safe) ───────────────
function dataURLtoBlob(dataURL) {
  const ci = dataURL.indexOf(',');
  const mime = dataURL.slice(5, ci).split(';')[0];
  const b64 = dataURL.slice(ci + 1);

  // FIX: chunk size must be multiple of 4 to avoid atob() mid-group errors
  const CHUNK = ATOB_CHUNK; // defined in decoder.js (65536, divisible by 4)
  const parts = [];
  let offset = 0;
